...
export class SplitPanel extends HTMLElement {
	...
	render(){
		...
		this.shadowRoot.innerHTML = `
			<style>
				...
				:host([resizing]){ cursor: col-resize; }
				:host { grid-template-columns: var(--first-size, 1fr) max-content var(--second-size, 1fr); }
	...
	resizeDrag(e){
		...
		if(this.direction === "column"){
			...
			const median = this.dom.median.getBoundingClientRect().height / 2;
